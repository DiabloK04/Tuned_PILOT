...
def plot_compare_cart_pilot(cart_model, pilot_tree, X_train, y_train, X_val=None, y_val=None):
    ...
    x_tr = X_train.iloc[:, feat_id];  y_tr = y_train
    x_va = X_val.iloc[:, feat_id] if X_val is not None else None
...
def plot_pilot_feature_importance(pilot_model, X_train, threshold=0.01):
...
def plot_prediction(model, nr, name, a=6, n_useless=0):
    grid_size = 50
