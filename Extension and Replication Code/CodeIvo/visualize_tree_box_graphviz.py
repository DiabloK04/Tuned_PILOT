...
def format_model(model_terms):
    """Format the cumulative linear model for display in leaf nodes."""
    terms = []
...
def visualize_tree_box_graphviz(model_tree, output_path="pilot_tree.png"):
    ...
    Edge labels indicate the linear boost added at each split, and leaf node labels
    show the full aggregated linear model.
    """
    G = nx.DiGraph()

    def add_nodes_edges(node, parent_id=None, model_terms=None, is_left=None, incoming_edge_label=None):
        # 1) Initialize or copy cumulative model terms
        cum_terms = {} if model_terms is None else model_terms.copy()

        # 2) Create a unique ID for this graph node
        current_id = str(uuid.uuid4())[:6]

        # 3) Build the node label (show split rule for decision nodes)
        label = f"{node.node}"
        if node.node not in ["lin", "con"] and node.pivot:
            label += f"\nX{node.pivot[0]} <= {node.pivot[1]:.2f}"

        # 4) If this is a leaf, append the full linear model
        ...
        # 6) Connect to the parent with the incoming edge label
        if parent_id:
            G.add_edge(parent_id, current_id)
            if incoming_edge_label:
                G[parent_id][current_id]["label"] = incoming_edge_label

        # 7) Recurse on the left child, computing its edge boost
        if node.left:
