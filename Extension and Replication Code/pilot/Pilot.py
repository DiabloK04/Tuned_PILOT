...
from sklearn.base import BaseEstimator, RegressorMixin, get_tags
...
@jit(nb.float64[:](nb.types.unicode_type, nb.int64, nb.float64[:], nb.int64[:], nb.float64))
def loss_fun(criteria, num, Rss, k: np.ndarray, bic_factor: float):
    """
    ...
    if criteria == "AIC":
        ...
        return num * np.log(Rss / num) + np.log(num) * k * bic_factor
...
def best_split(
    ...
    return best_feature, best_pivot, best_node, best_loss, lm_L, lm_R, interval, pivot_c
...
class PILOT(BaseEstimator):
    ...
    def __init__(
            ...
            regression_nodes_to_use: list[str] | None = None,
        ...
        self.fitted_ = None
        self.regression_nodes_to_use = regression_nodes_to_use
        self.max_depth = max_depth
        self.max_model_depth = max_model_depth
        self.split_criterion = split_criterion
        self.regression_nodes = REGRESSION_NODES if regression_nodes_to_use is None else regression_nodes_to_use
    ...
    def build_tree(self, tree_depth, model_depth, indices, rss):
        ...
        best_feature, best_pivot, best_node, best_loss, lm_l, lm_r, interval, pivot_c = best_split(
            indices,
            self.regression_nodes,
            self.n_features,
        ...
        # print(f"[PILOT TRACE] now at tree_depth = {tree_depth}")
        # # ——— DEBUG: at depth 3, print every model’s BIC and chosen feature ———
        # TARGET_DEPTH = 0  # change to whatever depth you want
        # if tree_depth == TARGET_DEPTH:
        #     print(f"\n[PILOT DEBUG] depth={tree_depth}  → overall = ({best_node}, feature {best_feature})\n")
        #     for m in ["con", "lin", "pcon", "blin", "plin"]:
        #         bf, bp, bn, bic, *_ = best_split(
        #             indices,
        #             [m],  # only this model type
        #             self.n_features,
        #             self.sorted_X_indices,
        #             self.X,
        #             self.y,
        ...
        #             self.min_unique_values_regression,
        #         )
        #         print(f"  Model={m:5s} | feature={bf:>2d} | BIC={bic:.3f}")
        #     print()
        # # ————————————————————————————————————————————————————————
    ...
    def get_params(self, deep=True):
        """
        Get parameters for this PILOT estimator.

        Parameters
        ----------
        deep : bool, default=True
            If True, returns parameters for this estimator and
        ...
        params = {
            ...
            'bic_factor': self.bic_factor,
        }

        # Currently, PILOT likely doesn't contain nested estimators,
        # but if it does in the future, you would include:
        if deep:
            for attr_name, attr_val in params.items():
                if hasattr(attr_val, 'get_params'):
                    deep_items = attr_val.get_params(deep=True).items()
                    params.update((attr_name + '__' + k, val) for k, val in deep_items)
    ...
    def fit(
        ...
        self.fitted_ = True  # a simple flag
