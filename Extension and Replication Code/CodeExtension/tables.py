...
File: tables.py
Author: Ivo Klazema
Description: This file makes the tables (in csv format) that are shown in the paper: "Extending PILOT by tuning the BIC penalty".
"""

# Load and prepare data
df_main = pd.read_csv('tablefiles/summary_PILOT_500times2_output.csv')
df_mars = pd.read_csv('tablefiles/summary_PILOT_two_settings_250_output.csv')
...
# Wilcoxon: PILOT vs. PILOT-k on nISE
...
# Mean std table: means + stds in parentheses on separate rows
mean_std_rows = []
...
# Percent increase from clean (0) to noise (1) table
